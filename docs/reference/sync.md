...
PRAGMA cache_size = -32000;       -- ⚠️ 32 MB, pending measurement (below)
PRAGMA mmap_size = 134217728;     -- ⚠️ 128 MB, pending measurement (below)
```

⚠️ **`cache_size` and `mmap_size` are marked pending, deliberately.** Under `ncruces/go-sqlite3`
it is undetermined whether `cache_size` is per-connection or shared, and whether `mmap_size` has
any effect at all. With a read pool of `NumCPU*2` connections plus one writer, a per-connection
32 MB page cache is a very different memory profile from a shared one. Measure in the arm64 spike
(ARCHITECTURE §13) before treating either as tuned.
