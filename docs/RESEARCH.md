...
| **MusicBrainz** | music | none (**UA required**) | ✅ **1 req/s per IP**; over → HTTP 503 | CC0 core data. **`User-Agent: App/version ( contact )` is MANDATORY.** Offers a **Live Data Feed + full local replica** — the escape hatch from 1 req/s | MBID (UUID) for artist/release-group/release/recording/work |
...
| **Last.fm** | music | API key | ≤5 req/s per IP averaged over 5 min 📄 | Good for play counts/similarity; **name-keyed not ID-keyed — a real weakness** | artist/album names |
