...
| **MusicBrainz** | No | ✅ **1 req/s per IP**, 503 above | Core data **CC0**. A descriptive `User-Agent` with contact info is **mandatory** ([docs](https://musicbrainz.org/doc/MusicBrainz_API/Rate_Limiting)). 1 req/s is not viable for library-scale scanning; the escape hatch is a [Live Data Feed](https://musicbrainz.org/doc/Live_Data_Feed) mirror — ✅ **terms verified 2026-08-16: the replication packets are CC BY-NC-SA 3.0, not CC0** (core data is CC0; the feed is not), **non-commercial users get a free access token and commercial users must contact MetaBrainz**, so the token is an operational prerequisite rather than an optional extra. Note also that exceeding 1 req/s **declines 100% of requests, not the excess**, and MusicBrainz asks explicitly that applications not poll and that calls be made at random intervals — the provider scheduler needs jitter, not a cron. **Do not route through `api.lidarr.audio`** — Servarr-run cache with documented lag and repeated breakage. |
...
`PRAGMA integrity_check` fails the same way and is not a second opinion. **Start UsArr once** so it
applies its migrations, or take the backup with a `sqlite3` ≥ 3.47.0; a database migrated past
