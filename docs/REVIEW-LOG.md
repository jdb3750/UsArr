...
| # | Decision | Why it cannot be settled below him |
|---|---|---|
| **8** | **The light-theme warning glyph's value** (V-06, §5.4.2). The *rule* is applied — the glyph is a non-text graphic at 3:1 and may carry more chroma than the word — and `#a9700a` is recorded in `tokens.css` as a computed candidate that clears the floor. It is **not landed** | The arithmetic clears it and the arithmetic is not the test. It needs a look at both themes at thumbnail scale, after the webfont actually loads, and that judgement is the owner's |
