...
| # | Finding | Disposition |
|---|---|---|
| **SW-05** | **The light-theme warning colour is a muddy brown and should be orange.** The owner: *"kind of eh… going more orange would be better for warnings."* `--status-warn` was `#8a5300` light / `#e0a33a` dark. This closes **round-5 item 8**, which escalated the same value to him and recorded `#a9700a` in `tokens.css` as a computed candidate | **Applied, and the escalated candidate is rejected on measurement rather than adopted.** `#a9700a` was never a *text* value: **3.98:1** on the page ground but **3.28:1** on `--selected #e7e3dd`, so it fails 4.5:1 on the ground that binds and could only ever have been a glyph token. What ships is **`#a44c00` light / `#fb9349` dark**, and both halves of the brief are met by one value rather than by a split. **Genuinely orange, measured:** `#a44c00` is CIE Lab hue **58.0°** at chroma **62.1** and `#fb9349` is **58.6°** at chroma **64.0**, against CSS `orange` `#ff7f00`'s **59.6°** — so the two themes are now the same hue at two lightnesses, where before they were a brown (70.4°) beside a gold (77.8°). **Readable as text on the worst of all five grounds:** 5.53 page · 5.11 surface · 4.84 hover · **4.55 selected** · 5.67 inset in light; 8.07 · 7.52 · 6.92 · **6.57** · 8.55 in dark. Dark holds chroma (64.0 vs 61.9) and worst-ground contrast (6.57 vs 6.63) where they were and moves only the hue. **The word/glyph split §3.2 grants is therefore NOT taken** — it stays documented and unexercised, because a second token buys chroma the word already has. Rejected en route and recorded so they are not retried: `#c98a00` (2.80), `#b8860b` (3.09), `#a9700a` (3.28), and `#b04300` — more chromatic and still clearing 4.50, but ΔE76 **18.5** from `--status-error` where `#a44c00` sits at **25.2**. `tokens.css`, `mockups/usarr.css` and DESIGN-DIRECTION §3.2 changed in one commit so they cannot drift, and `docs/design/check.mjs` now asserts the drift and the ratios on every run. ⚠️ **Correction, 2026-08-17 — the no-drift property above is asserted over a file set that omits the file that then drifted.** The three files named are the three that were co-edited; **`web/src/app.css` carries the same tokens and is not among them**, and it is the file that went stale. `--status-warn` held the superseded `#8a5300` light / `#e0a33a` dark there in **three** places — `:root`, `:root[data-theme='dark']` **and** the `prefers-color-scheme: dark` block, the third found only by parsing per theme block rather than per file — until `4f578fd`, *"fix: app.css picks up the owner's `--status-warn` decision — #a44c00 / #fb9349"*, merged as `6bac810`. **The poison test is the hard evidence, and it was run both ways round.** Raised by the frontend thread's sweep and recorded in `4f578fd`'s own message; re-run here on `6bac810` with the tree otherwise clean, instrument `node docs/design/check.mjs` driven by `make design` (`Makefile:588`). `--status-warn: #ff00ff` substituted in `web/src/app.css`'s `:root` left the checker **green — `check.mjs` exit 0**, closing on `all design checks pass`. The same `#ff00ff` substituted on `--st-warn` in `docs/design/mockups/usarr.css` failed it — **`check.mjs` exit 1** (surfaced by make as `Error 1`, `make` itself exiting 2), reporting `FAIL  token drift (light)` and the light-ground contrast floor, `2 FAILURES`. The cause is the scanned set, `check.mjs`'s `const SOURCES`: `docs/design/tokens.css`, `mockups/usarr.css`, `mockups/fonts.css`, `mockups/usarr.js` and the mockup HTML. Nothing under `web/` is read as CSS — the only `web/` paths in the file resolve Playwright — so the checker is **structurally blind to the shipped application in both directions**: it neither catches a value that has drifted there nor notices one introduced there. **The distinction, which is why this is amended rather than simply corrected:** *"changed in one commit so they cannot drift"* is a **convention, not a mechanism**. It holds only while people keep doing it, it binds only the files somebody remembered to list, and it was silently false for a fourth file nobody had listed — with nothing anywhere to report the gap. **Not overstated:** SW-05 was correct about the three files it named, they did change together, and co-editing the canonical token sources is a reasonable practice. The defect is that a **scoped convention was written in the language of a guarantee**. The vitest palette test now in flight on the frontend thread is the first actual mechanism here, and it covers **two of the four** files |
...
| **SD-02r** | `docs/design/mockups/README.md:269` | *"`prototype.html`'s `<title>` read … **It is now** *"UsArr screen mockups: static, invented data, nothing implemented"*"* | ⚠️ **CORRECTED — verdicted True at `0656bd9`, and FALSE at `a29a07f`, which is roughly six hours later.** 📌 **The commit that invalidated it is `c2cefa3`, *"docs(design): the same status claim, in the two places it was not reported"* — named because "false at `a29a07f`" says where the flip was *noticed* and not where it *happened*, and only the latter is re-findable.** `git log --oneline -L6,6:docs/design/mockups/prototype.html` returns `c2cefa3` as the sole change to that line since the verdict was taken, and `git merge-base --is-ancestor c2cefa3 0656bd9` is **false**, which is the check that proves the original reading was correct when it was made rather than merely asserted to be. ℹ️ `c2cefa3`'s own message closes the loop from the other side: *"`docs/design/mockups/README.md:269` still presents the old title as current and is left for the pass that owns that file"* — the design area knew it was leaving this row false and said so, so the defect is a hand-off that was never picked up, not a silent decay. The original reading stands as taken: `prototype.html:6` did match byte for byte. It no longer does — the design area's own sweep (logged above `DS-07 and DS-14`, and it flagged this row itself) extended `check.mjs` to `document.title` and the tag is now `<title>UsArr screen mockups: static, invented data</title>`, **without** *"nothing implemented"*. So the README's *"It is now"* quotes a title that no longer exists. 🚩 **This was the row called "the weakest in the table" and "NOT the same defect", and it is the only row that has since gone false — which makes it the entry's own thesis demonstrated on the entry, inside one working day.** ℹ️ **The reasoning that filed it as an exception was not wrong**: a dated record of an edit *is* a genre allowed to restate. What the reasoning missed is that this one restates in the **present tense** — *"It is now"* — and a present-tense quotation of a live string is a status claim wearing a historian's coat. **The fix is one word: past-tense it**, so the changelog records what the title was changed *to* at the time without asserting what it is *now* | **`prototype.html`** itself | 🔶 **Claimed by the design area**, which is running this README as its own pass — `SD-01a` and the `document.title` guard already landed here |
...
**The inversion, carried here from the design area because `SD-01`'s rule is only half an answer.**
`SD-01`'s advice to every row above is *assert no status at all, route the reader to §16 and the
tree*. That works because none of these sites **needs** to carry a status. 🚩 **But some strings
must, and for those "route the reader elsewhere" is not available** — the mockup notice is the case
in point. `DESIGN-DIRECTION.md` bans fabricated data in any shipped surface and grants exactly one
exception, *"a design mockup, **which must say on every page that its data is fabricated** — not
only in a sibling README, because the single-file published build is detached from it"*. The label
**is** the condition the exception is granted on, so it cannot be replaced by a pointer to §16
without withdrawing the exception. **For strings like that, `SD-01a`'s own diagnosis is the
instruction: the stale sentence *"was not pinned; it was **unguarded**"*** — `grep -rn "none of
these screens"` returned the five source pages, the generated `prototype.html` and the mockups
README, and **no check at all** — **so it went stale because nothing asserted it, and no amount of
careful wording would have changed that.** ⚠️ **A more carefully worded sentence decays on the same
schedule as a careless one; only something that runs can notice.** ✅ **The durable fix is therefore
a guard that fires, and `docs/design/check.mjs` is the worked example:** it now asserts, **per
install** — the loop runs over both, so neither variant can drift alone — that the notice *describes
the install that is selected*, that it *names its data as invented*, and that it *makes no
implementation-status claim*, plus the third property again over `document.title`. **Each is a
property, so any honest rewording survives it**, which is the whole of `SD-01a`. 🚩 **And they were
fired deliberately before being trusted, per `CLAUDE.md`'s *"one that has never been triggered is
indistinguishable from no guard"*: the notice assertions exit 1 with exactly `4 FAILURES` (two
properties × two installs) when the old *"which is pre-alpha software"* wording is restored, and the
title assertion exits 1 with `1 FAILURE` naming `"implemented"` when the old `<title>` alone is put
back — with all six notice assertions staying green through that second probe, which is what
establishes the two guards are independent rather than one covering the other.** **So: `SD-01`'s
rule and its complement, and a site takes whichever applies — where a claim does not need to live
locally, delete it and name the owner; where it genuinely must, guard the property and fire the
guard.** ℹ️ **Nothing in this paragraph changes a row above.** Every one of the twenty-one is
unguarded prose with no natural place to put an assertion, which is why `SD-01` and not `SD-01a` is
their parent; the complement is recorded so the *next* site with somewhere to put a check is not
argued about from scratch.
...
### Two sites the inventory missed entirely, both already fixed

**Same §6.1 mechanism, and folded in here rather than opened as a second amendments block** — one
entry with two places to look for amendments is the defect this entry is about, wearing a different
hat. Nothing above is renumbered or reworded; both **take the next free id in the `SD-02` prefix**,
so nothing collides; both are quoted from the file at **`0656bd9`**, the same pin the table uses, and
each names the commit that settles it. 🚩 **These are a GAP, not decay, and the difference is the
reason they are worth their space.** `SD-02r` was *found and verdicted* and the verdict expired.
These two were never looked at: the compilation walked prose and rendered text, so it saw neither a
CSS file's comment header nor an HTML `<head>`. **A row that went false is a shelf-life problem and
re-reading the table finds it; a site that never got a row is a coverage problem and re-reading the
table cannot find it, because the absence looks exactly like completeness** — which is `SD-02e`'s
predecessor, *"the roster read as COMPLETE while omitting `indexer_catalog`"*, committed by the
inventory that quotes it.

| # | Site, at `0656bd9` | The claim, quoted | At `0656bd9` | Authoritative for that fact | State |
|---|---|---|---|---|---|
| **SD-02v** | `docs/design/tokens.css:4-6`, the file header | *"Status: design document, pre-alpha. **None of these tokens is implemented.** A `web/` directory exists and carries a SvelteKit shell, but **it does not consume this file.**"* | 🚩 **FALSE, and it had been false since the shell landed.** `git show 0656bd9:web/src/app.css` opens *"LAYER 1 TOKENS — `docs/design/tokens.css`, **ported here under its own canonical names. Every value is verbatim from that file**"*, and that text arrived with `a279517`, *"feat: land the v0.1 core packages and embedded web shell"*. Both halves were wrong: the tokens were implemented, and `web/` did consume the file. ℹ️ **The same false half as `SD-01a`'s notice** — *"none of these … is implemented"* — in a third location, which is what made it findable only by looking rather than by re-reading | `ARCHITECTURE.md` §16 for the milestone; **`web/src/app.css`** for whether anything ports the tokens | ✅ **Fixed by `c2cefa3`**, *"docs(design): the same status claim, in the two places it was not reported"* (`git log --oneline -- docs/design/tokens.css`). The header now routes to §16 and the tree and keeps only the half that does not decay: *"nothing IMPORTS this file … so this file being canonical is a review rule rather than a build dependency"* |
| **SD-02w** | `docs/design/mockups/prototype.html:6`, the `<title>` | *"`<title>UsArr screen mockups: static, invented data, **nothing implemented**</title>`"* | 🚩 **FALSE**, by the same reading against the same tree — `git ls-tree -r 0656bd9 web/src/routes/` counts **seven** `+page.svelte` routes. ⚠️ **A distinct site from `SD-02r`, which is why it takes its own id rather than being folded into that row**: `SD-02r` is the README *quoting* this string, and applying `SD-02r`'s one-word fix would have left the string itself asserting the same status to every browser tab, bookmark and history entry. 🚩 **It is the one user-visible string a rendered DOM walk cannot see** — `check.mjs` §13's copy corpus counts `document.title` but never reads it — which is exactly why an inventory assembled from prose and rendered text missed it | `ARCHITECTURE.md` §16 and the tree. ℹ️ The string is **generated**, so **`build_prototype.py`**'s f-string owns it and the HTML does not — editing `prototype.html` would have been overwritten by the next build | ✅ **Fixed by `c2cefa3`**, at the f-string, and — unlike every other row in this entry — now **guarded**: see the inversion paragraph above |

**Counts, restated because a stale count inside this entry would be the joke telling itself.** The
heading's *"twenty-one remaining sites"* is **unchanged, deliberately**: both additions are ✅ Fixed,
so neither is remaining, and the eighteen-still-open figure above is unaffected. What changes is the
...
# NOCI-02 — the design thread's half of NOCI-01, sorted site by site rather than swept

**Date:** 2026-08-17. **Branch:** `claude/hearth-thread-vn9w7u`, cut from `origin/main` at `6bac810`
and merged forward over `2d3138a`, which is `NOCI-01`. **Takes the hand-off `NOCI-01` §NOCI.5 left**
— *"`docs/design/DESIGN-DIRECTION.md` (5) and `docs/design/check.mjs` (2) carry the same pattern …
**Not touched**: `docs/design/` and `ARCHITECTURE.md` §17 are the design thread's, per §11's area
map, and this is a routing note rather than an edit."* Routed here with the owner's instruction
attached. `NOCI-01`'s own three-way sort is adopted unchanged; what this entry adds is the
disposition of the seven sites it declined to touch, and one fix that is not wording at all.

**The site list was re-established rather than relayed**, per §11 rule 5's neighbour on absence
claims. `grep -rnE '\bCI\b|continuous integration|GitHub Actions' docs/design/` on `4696e58` returns
**exactly seven** hits — five in `DESIGN-DIRECTION.md`, in §7.2, §9.1, §9.2, §11 and §13, and two
in `check.mjs`, in the Playwright-resolution note and in the headless-shell check's comment — the set `NOCI-01` named, confirmed on this tree
rather than taken on report.

## NOCI-02.1 The seven, each judged on its own

The test applied is the one `NOCI-01` §NOCI.2 sets and the owner restated: **specification is kept,
assertion is fixed.** Wording that says what a CI *must* or *should* do when one exists is design
intent, is not a claim about today, and is not false; wording that states what a CI *does* is false
in a repo that has none, and is the `SD-01` shape — a document restating a fact about tooling it does
not own. Where the intent is sound but the grammar reads as present-tense fact, the grammar is
tightened and the intent kept.

| Site | Text | Judged | Why |
|---|---|---|---|
| `DESIGN-DIRECTION` §7.2 | *"it belongs in the `EXPLAIN QUERY PLAN` assertions in CI (§13 of ARCHITECTURE)"* | **Specification — kept** | The verb is *belongs*: it says where a query-plan assertion should live, not that one is running. It also **names the document that owns the fact** — ARCHITECTURE §13, which `NOCI-01` §NOCI.2(c) already dispositioned as design intent — which is `SD-01` being obeyed rather than broken. Rewriting it here would restate §13's status from a file that is not authoritative for it |
| `DESIGN-DIRECTION` §9.1 | *"**CI asserts it**, and it is cheap: no element's `getBoundingClientRect().right` may exceed `innerWidth` on any screen × state × width triple"* | **Assertion — fixed** | 🚩 **The sharpest of the seven, because the assertion is real and only the actor was wrong.** `check.mjs`'s `4. Overflow: nothing past the viewport, nothing past its own .tablewrap` runs exactly this test — `b.right <= window.innerWidth + 0.5`, plus a `.tablewrap` shear check — and `make design` reports it on this tree as `overflow at 390px … 110 screen×state×install×panel combinations (floor 104)`, repeated at 1280/1440/1680/1920. So a reader who took the sentence at its word would look for a CI, find none, and conclude the rule is unguarded, when in fact it is guarded harder than the sentence claims. Now reads *"`docs/design/check.mjs` asserts it"*, with the correction stated in place and `NOCI-01` / `OPTIN-01` cited |
| `DESIGN-DIRECTION` §9.2 | *"§11's CI assertion has nothing to run over until one appears"* | **Grammar tightened, intent kept** | The sentence's content is correct and is itself an anti-invented-status point — the `dominant_color` rule has no call site, so its guard has nothing to run over. Only the word `CI` presupposes an actor. **One word deleted**: *"§11's assertion has nothing to run over until one appears."* Nothing else in the passage changes |
| `DESIGN-DIRECTION` §11 | *"**Asserted in CI over any computed-fill / foreground pair that ships in a fixture**"* | **Grammar tightened, intent kept** | This is a *standing guard* specified deliberately — the passage argues, at length and correctly, that a conditional rule needs a guard that predates its first call site. That intent is untouched. The bold clause was in the present indicative for a guard that runs nowhere, so it now reads *"**To be asserted over any computed-fill / foreground pair that ships in a fixture** — in the design gate, and in a CI if one is ever added"*, which is the same instruction with no claim about today attached |
| `DESIGN-DIRECTION` §13 | *"`[grep]` = enforceable as a CI grep, ESLint or Stylelint rule"* | **Specification — kept** | *Enforceable as* is explicitly potential, and the three alternatives make it a statement about the **shape** of a rule — mechanically checkable versus needing judgement — not about which tool checks it. The contrast is with `[review]` on the same line. There is no present-tense claim to falsify |
| `check.mjs`, the Playwright-resolution note | *"`make design` died with a raw ERR_MODULE_NOT_FOUND on the owner's machine **and in CI**"* | **Assertion — fixed** | A claim about where this script has run. It has never run in a CI, so the failure it describes never happened there. Now *"on the owner's machine and on any other"*, which is the true and more general form — the defect was that an out-of-tree absolute path resolves nowhere but the one container, and that needs no CI to bite |
| `check.mjs`, the headless-shell check | *"would measure a different renderer **from CI** and from every other machine"* | **Assertion — fixed** | Same shape, one layer subtler: it names a CI as a **surface measurements are taken on**, which is §11 rule 5's own subject matter appearing inside a comment that is otherwise about surface comparability. Now *"from every other machine this has ever been run on"* |

**Two kept, five fixed** — and of the five, **three are grammar or actor, not intent**: no rule, guard
or budget is weakened by any edit here, and no assertion is added or removed. `check.mjs`'s two are
comments; its executable content is byte-identical.

## NOCI-02.2 The one that is not wording: a guard that misdescribed its own invocation

`check.mjs`'s comments are the reason this entry is not a tidy-up. **`OPTIN-01` found that nothing
invokes `make design`; this is the same defect seen from inside — the checker asserting that it runs
somewhere it does not.** A reader auditing the guard would have taken *"and in CI"* as evidence the
guard is mechanised, which is precisely the belief `OPTIN-01` exists to correct, and they would have
taken it **from the guard itself**, which is the most credible witness in the room.

So the fix is not only the deletion. `check.mjs`'s header now carries a **`WHAT INVOKES IT: nothing`**
block naming the real answer — a person or an agent types `make design` — with `NOCI-01` cited for
the absence of a CI and `OPTIN-01` cited for the absence of any document requiring this target, and
recording that the two comments below it were corrected and why.

⚠️ **This is at least the third form the same pattern has taken in this repo**, and they are worth
naming together because each was found by a different route and none would have found the others:

1. **A guard that cannot speak.** `DEVELOPMENT.md` §11 rule 3 — `make design`'s guard died at its own
   variable assignment under `.SHELLFLAGS := -eu -o pipefail`, before reaching the error message it
   existed to print.
2. **A guard nothing invokes.** `OPTIN-01` — the design guards are opt-in, and nothing in the tree
   runs them.
3. **A guard that misdescribes its own invocation.** This entry.

A fourth, adjacent and already logged, is **a guard that cannot see the file it is guarding**: the
`SW-05` correction above, where `check.mjs`'s scanned set omits `web/src/app.css` and a `#ff00ff`
poison there leaves the checker green at exit 0. Between them the four say something the individual
findings do not: **in this repo, "there is a check for that" has been wrong in four different ways,
and only one of them was about the check's logic.**

ℹ️ **The relayed count was three; the real count was nine.** `OPTIN-01` handed off *"three sites"*;
`NOCI-01`'s own grep found **nine** in category (a) alone, plus the seven dispositioned here. **That
is the fourth undercount this week, every one of them from a search narrower than the claim it
supported** — and it is exactly what §11 rule 5 and its neighbours exist to prevent: an absence or a
count carries the roots it searched, or it is not a count, it is an impression. This entry states its
own root (`grep -rnE … docs/design/` on `4696e58`) for the same reason.

## NOCI-02.3 On the gate

`make design` was run on this tree — `/opt/node22/bin/node` **v22.22.2** — **exit 0, 126.6 s wall,
83 `ok` lines**, closing on `all design checks pass`. ⚠️ **Its reach over this entry is partial and asymmetric, and the asymmetry is the
point.** What it genuinely covers is the §9.1 fix, and by execution rather than by reading: `make
design` **runs** `check.mjs`, so the edited file is parsed and executed, and its `4. Overflow`
section reported `110 screen×state×install×panel combinations (floor 104)` at each of five widths —
which is the evidence for the replacement claim *"`docs/design/check.mjs` asserts it"*. What it does
**not** cover is everything else here. `check.mjs`'s own text is not linted by it: the §13 ban sweep
runs over `const SOURCES` — the two stylesheets, `usarr.js` and the mockup HTML — and `check.mjs` is
not in that list, so the comments corrected above are outside its own reach in exactly the way
`SW-05` describes for `web/src/app.css`. And **no check in the file opens `DESIGN-DIRECTION.md`** —
its only `readFileSync` targets past `SOURCES` are `prototype.html`, `tokens.css`, `usarr.css` and
