...
**Left to other threads, not touched:** `ARCHITECTURE.md` §17 and everything under `docs/design/`.
§17.8's *"a library for Audiobookshelf / Komga / Navidrome (Kavita from v0.2, Jellyfin from v1.0)"*
now disagrees with §16 and needs the same re-sequencing; the rest of §17's Komga mentions are UI
copy and degradation examples that state no milestone and are correct as written.
