 ...
 *
 * THE HOLE THIS CLOSES. Every rendered check below drives prototype.html, and
 * prototype.html is GENERATED by build_prototype.py from the eight mockup files
 * plus the script's own template text. Nothing verified the two were in step,
 * so the rendered half of this gate was blind to edits in the files humans
 * actually edit. Measured before this landed, on origin/main at 9ba240b: a §13
 * banned word injected into a static <h2> of index.html left `make design` at
 * exit 0, "all design checks pass", with the user-visible string count
 * unchanged at 6932 -- while the same word in prototype.html failed §13 copy
 * with 8 violations. The gate was guarding the artifact and ignoring the source.
 *
 * WHY A DIGEST STAMP AND NOT REGENERATE-AND-COMPARE. Regenerating here would be
 * the most direct proof, and it would make python3 a hard requirement of `make
 * design`. It is not one today: python3 appears nowhere in the Makefile, and
 * this target's three preconditions are node, this script and a Playwright
 * browser cache. Growing an interpreter dependency to check a docs directory is
 * a real cost -- the same argument that keeps `make check` free of Docker -- so
 * the proof is taken at GENERATION time, where python3 is already running, and
 * only a comparison is left here. node:crypto is stdlib, so this adds nothing.
 *
 ...
 * so". A manifest sitting beside the sources invites exactly that -- it reads
 * as a file that wants updating. The digests instead live inside the generated
 ...
 * there: this asserts the INPUTS are unchanged since the last generate. It does
 * not re-derive the output, so it cannot catch a prototype.html whose body was
 * hand-edited while every input stood still. That is what the rest of this file
 * is for -- every rendered check below opens prototype.html directly, so a
 * hand-edit surfaces as whichever defect it introduced. The two halves are
 * complementary, which is why this is added beside them and replaces none.
 * ========================================================================== */
head('1e. Provenance: prototype.html is in step with its sources');
(function buildStamp() {
  const REGEN = 'python3 docs/design/mockups/build_prototype.py   (from the repo root)';
  const raw = readFileSync(join(MOCKUPS, 'prototype.html'), 'utf8');

  const m = raw.match(/BUILD STAMP\b([\s\S]*?)END BUILD STAMP/);
  if (!m) {
    fail('provenance: prototype.html carries no build stamp, so nothing here can tell whether it ' +
      'is in step with its sources. Regenerate with: ' + REGEN);
    return;
  }
  const entries = [...m[1].matchAll(/^\s*(\S+)\s+sha256:([0-9a-f]{64})\s*$/gm)]
    .map((e) => ({ name: e[1], want: e[2] }));

  /* Floor on the corpus, not on the hit count. A ban-shaped check wants zero
     hits, so a zero here is ambiguous: it is also what a renamed marker or a
     changed stamp format produces, and that would report a clean pass having
     compared nothing at all. Nine inputs today — five screens, fonts.css,
     usarr.css, usarr.js, and build_prototype.py, whose own bytes count because
     the doctype, <head>, <title> and banner are template text living there. */
  if (!floorOk('provenance', entries.length, 9, 'recorded input digest(s)')) return;
  ...
  for (const { name, want } of entries) {
    ...
    try {
      ...
      drifted.push(`${name}  — stamped as an input but not readable in the tree`);
      continue;
    }
    if (got !== want) drifted.push(`${name}  — on disk ${got.slice(0, 16)}…, stamped ${want.slice(0, 16)}…`);
  }
  ...
  const unstamped = FILES
    .filter((f) => dirname(f.path) === MOCKUPS && !stamped.has(basename(f.path)))
    .map((f) => `${rel(f.path)}  — scanned by this gate but named by no stamp entry`);

  if (drifted.length || unstamped.length) {
    fail(`provenance: prototype.html is STALE — ${drifted.length} of its ${entries.length} inputs have ` +
      `changed since it was generated` +
      (unstamped.length ? `, and ${unstamped.length} scanned source(s) are named by no stamp entry` : '') +
      `. Every rendered check below opens prototype.html, so until it is regenerated they are ` +
      `measuring the OLD copy and any edit to the file(s) named here is unguarded. ` +
      `This is not a finding about the source's content — it is a finding that the source was ` +
      `never built in. Regenerate with: ${REGEN}`);
    [...drifted, ...unstamped].forEach((d) => note(d));
  } else {
    ok(`provenance: prototype.html is in step with all ${entries.length} of its inputs ` +
      `[sha256 re-hashed from disk and compared against the stamp it was generated with, floor 9]`);
  }
})();
