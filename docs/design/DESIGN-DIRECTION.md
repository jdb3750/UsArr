...
screen×state×install×panel combinations swept, rendered chrome and cell strings read, list renderings
...
| `BOOM! Studios`, against *no `!` in UI strings* | The copy bans are evaluated over rendered text and attributes **minus whatever the markup itself declares to be a substituted value**: `data-copy="data"`, on the element that holds it. Still never by name — `check.mjs` knows no publishers, and the next one needs no edit to it — but declared **per site** and **counted**, with the number of skipped strings printed beside the corpus total, so an opt-out is a thing the sweep has seen and dismissed rather than a thing it never found. That is the same argument the roving check makes for collecting `[data-roving-optout]`. ⚠️ **This row used to read "outside any `<td>`", and justified that on prose being kept out of cells by the row-height check. It was false.** Row height is a ceiling, not a ban on prose: it is held deliberately loose on Home, Search and Requests (60/80/80 px against 49 on Services and Libraries) and it skips Services' `annex` state. Reading cells took the copy corpus from **6,908 strings to 13,411** — the missing half was the product's own voice, ungated since the rule was written. The two rules never held each other up; this one now rests on its own declaration and on nothing else. |
