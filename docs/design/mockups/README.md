...
| **Recent grabs** | Requests | Nothing recorded that a grab had happened. The confirmation lived in the Title cell of a search-result row and search results are transient, so UsArr's only write path produced a 68 GB download with no UsArr-side record and no way to answer "did that one work?" — which sends the user back to Prowlarr's own UI to grab, the outcome this project exists to prevent. Six rows, the write queue's own vocabulary (`pending`, `inflight`, `verifying`, `done`, `failed`), and `done` states plainly that it means the download client accepted the release and nothing more. |
