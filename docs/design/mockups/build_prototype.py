#!/usr/bin/env python3
"""Generate prototype.html from the four screen files.

The four files are the source of truth. This script inlines usarr.css and
usarr.js, concatenates the four <main> blocks, and rewrites every link to the
four files into a #hash route so the single file clicks through the same way.
"""
import re, pathlib

SRC = pathlib.Path(__file__).resolve().parent
PAGES = [('home', 'index.html'), ('services', 'services.html'),
         ('search', 'search.html'), ('requests', 'requests.html')]
ROUTE = {'index.html': '#home', 'services.html': '#services',
         'search.html': '#search', 'requests.html': '#requests'}


def block(text, name):
    m = re.search(r'<!-- ' + name + r' START -->(.*?)<!-- ' + name + r' END -->', text, re.S)
    if not m:
        raise SystemExit('marker not found: ' + name)
    return m.group(1)


def rewrite_links(html):
    for fname, route in ROUTE.items():
        html = re.sub(r'href="' + re.escape(fname) + r'(?:#[A-Za-z0-9_-]+)?"',
                      'href="' + route + '"', html)
    return html


css = (SRC / 'usarr.css').read_text()
js = (SRC / 'usarr.js').read_text()
index = (SRC / 'index.html').read_text()

sprite = block(index, 'SPRITE')
topbar = rewrite_links(block(index, 'TOPBAR'))
sidebar = rewrite_links(block(index, 'SIDEBAR')).replace(' aria-current="page"', '')

body = '\n'.join(rewrite_links(block((SRC / f).read_text(), 'PAGE:' + r).strip())
                 for r, f in PAGES)

out = f'''<!doctype html>
<html lang="en" data-density="compact">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>UsArr v0.1 screen prototype</title>
<style>
{css}
</style>
</head>
<body>
<!-- Generated from index.html, services.html, search.html and requests.html.
     Do not edit this file by hand: edit those four and regenerate. -->
{sprite}
<div class="app" data-sidebar="open">
{topbar}
{sidebar}
{body}
</div>
<div class="toasts" aria-live="polite"></div>
<script>
{js}
</script>
</body>
</html>
'''

(SRC / 'prototype.html').write_text(out)
print('wrote', SRC / 'prototype.html', len(out), 'bytes')
