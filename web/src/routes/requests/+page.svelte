<script lang="ts">
	...
	/**
	 * The same-tab half of `Search again`, and the reason the link keeps its href.
	 *
	 * The href itself is `$lib/requests`' `requestsSearchHref`, built inside
	 * `RecentGrabs.svelte`: §17.4 fixes `?q=` as the one way another surface hands
	 * a query to this screen, and there is one function that builds it rather than
	 * a copy per call site. It is what a middle-click, a ctrl-click and a
	 * right-click all act on, and it WORKS when they do — `onMount` seeds a search
	 * from `?q=`, so a new tab opened this way runs the search on its own.
	 *
	 ...
	 * middle, modified, and any non-primary button — which is what makes that href
...
<section class="section" id="recent-grabs">
	...
	{:else}
		<!--
			THE TABLE ITSELF IS `$lib/RecentGrabs.svelte`, and Home draws the same rows
			from the same component under its own three-column projection. Every string
			on a row is `grabOutcome`'s, and the markup that renders them is one file
			rather than two — which is what keeps `requests.test.ts`'s ban list over the
			grab vocabulary covering both screens instead of whichever one it was
			written against.

			SIX COLUMNS HERE, THREE ON HOME. This is the canonical block: it carries
			the indexer, the protocol and the size, and it is the only one that offers a
			row an action (§17.3 — a problem is stated canonically once per screen).
		-->
		<RecentGrabs
			{grabs}
			columns={grabColumns}
			...
			actions={{ onSearchAgain: searchAgainFrom }}
		/>
	{/if}
</section>
