 ...
 *
 * Currently empty, and it got there the way the comment above describes: the
 * one entry was --toolbar-h, app.css stopped declaring it when the shell became
 * a viewport-height grid, and the design thread then deleted it from tokens.css
 * too. The entry and that deletion are one inseparable change — either half
 * alone fails this suite — so they landed in a single commit.
 */
